...
window.onload = () => {
  ...
  document.getElementById("prodSearch").addEventListener("input", () => {
    fetch(
      "https://dashabhujamain1.azurewebsites.net/products/search?q=" +
  ...
  fetch("https://dashabhujamain1.azurewebsites.net/products/getall", {
    method: "GET",
    headers: {
...
function registerInterest(event) {
  ...
  div.setAttribute(
    "style",
    "background-color: #98234b; padding: 10px; border-radius: 5px; color: white"
  );
  ...
  fetch("https://dashabhujamain1.azurewebsites.net/products/interested", {
    method: "POST",
    headers: {
      "Content-Type": "application/json",
      Authorization: token,
    ...
    .then((res) => {
      ...
      if (res.message) {
        ...
        document
          .getElementById(`interest-cover-${prodId}`)
          .removeChild(document.getElementById(`registering-${prodId}`));
