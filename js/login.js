...
window.onload = () => {
  ...
  document.getElementById("forgotpassword").addEventListener("click", () => {
    Swal.fire({
      ...
      if (result.isConfirmed) {
        fetch("https://dashabhujamain1.azurewebsites.net/auth/forgotpw", {
          method: "POST",
  ...
  document.getElementById("sub").addEventListener("click", function (e) {
    ...
    if (re.test(email.trim())) {
      ...
      document.getElementById("label1").innerHTML = "Please enter valid email";
    ...
    if (status.includes("false")) {
      ...
      fetch("https://dashabhujamain1.azurewebsites.net/auth/login", {
        method: "POST",
        headers: new Headers({ "content-type": "application/json" }),
        body: JSON.stringify({
          email: email,
