...
function handleSOS() {
  fetch("https://dashabhujamain1.azurewebsites.net/register/sos", {
    method: "POST",
    headers: {
...
window.onload = () => {
  ...
  function showPosition(position) {
    ...
    fetch("https://dashabhujamain1.azurewebsites.net/visit", {
      method: "POST",
      headers: {
        "content-type": "application/json",
      },
    ...
    fetch(
      `https://api.opencagedata.com/geocode/v1/json?q=${position.coords.latitude}+${position.coords.longitude}&key=5db5a96f174d46e389853c0ec12d44a2`
    )
      .then((res) => res.json())
      .then((res) => {
        if (res.results[0].components.state) {
          document.getElementById(
            "sos"
          ).innerHTML = `<a onclick="handleSOS()" href=tel:${
            store[res.results[0].components.state.toUpperCase()]
